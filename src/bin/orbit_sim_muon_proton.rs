...
fn main() -> std::io::Result<()> {
    ...
    let m1 = physical_constants::MUON_MASS;
    ...
    let mut file = File::create("orbit_trajectory_muon_proton.csv")?;
    writeln!(file, "step,time,x,y,vx,vy,F,Er1,Ei1,Et")?;

    let e: f64 = physical_constants::ELEMENTARY_CHARGE;
    ...
    for i in 0..steps {
        ...
        if i % log_step == 0 {
            ...
            writeln!(file, "{},{:.4e},{:.6e},{:.6e},{:.6e},{:.6e},{:.6e},{:.6e}, {:.6e}, {:.6e}",
                ...
                interaction.er1.norm() / e,
                interaction.ei1.norm() / e,
                (interaction.er1 + interaction.ei1).norm() / e
            )?;
        }       
    ...
    println!("Simulation Complete. Data saved to 'orbit_trajectory_muon_proton.csv'");
    Ok(())
}