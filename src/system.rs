...
impl GeometricEncodedMedium {
    ...
    pub fn calculate_interaction(&self, p1: &GeometricKnot, p2: &GeometricKnot, d: Complex64) -> GemInteractionResult {
        ...
        // 1. Calculate Schwarzschild Radii (rs = 2Gm/c^2)
        let rsm1 = (2.0 * self.g * m1) / self.c.powi(2);
        let rsm2 = (2.0 * self.g * m2) / self.c.powi(2);
        
        // 2. Calculate Charge Radii (rq = md / (m * alpha^2)) -> Simplifies to d/alpha^2
        // Used for Natural Distance calculation
        let rqm1 = (m1 * d) / (m1 * self.alpha.powi(2));
        let rqm2 = (m2 * d) / (m2 * self.alpha.powi(2));

        // 3. Natural Distance (d_nat = (rq1 + rq2) - (rs1 + rs2))
        let d_nat = (rqm1 + rqm2) - (rsm1 + rsm2);

        // 4. Shadow Charges (q = Sqrt[4 * pi * epsilon_o * c^2 * m * rs])
        // This is the "Static" calculation logic from Mathematica.
        ...
        // 5. Curvature (kappa) & Gravitational Scaling (Go)
        // kappa = ((m1 + m2) / (q1 + q2)) * Xi
        ...
        // Force Calculation (Assuming F = m * a * SQRT_2 as per previous logic, pending user feedback)
        let f1 = (ag1+gm1) * Complex64::from(m1) * SQRT_2;
        let f2 = (ag2+gm2) * Complex64::from(m2) * SQRT_2;
        let force = (f1.powi(2) + f2.powi(2)).sqrt();

        // 8. Energy Allocation
        // Denominator is 2.0 (not SQRT_2) as per Mathematica snippet
